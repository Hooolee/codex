...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.codex-fork/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// Directory containing all Codex state (defaults to `~/.codex-fork` but can be
    ...
    /// Settings that govern if and what will be written to `~/.codex-fork/history.jsonl`.
    pub history: History,
